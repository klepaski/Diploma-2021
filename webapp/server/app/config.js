...
var pipeline = [
  ...
  function getDefaultSettings(context) {
    var file = fs.readFileSync(path.resolve(__dirname, '../defaults.yml'), 'utf8');
  ...
  function appURL(context) {
    if (process.env.APP_URL) {
      context.result.appURL = process.env.APP_URL;
    }
  },

  function setHTTPS(context) {
    if (process.env.HTTPS_KEY) {
      context.result.https.enable = true;
      context.result.https.key = process.env.HTTPS_KEY;
    }
    if (process.env.HTTPS_SERT) {
      context.result.https.enable = true;
      context.result.https.cert = process.env.HTTPS_SERT;
    }
    if (process.env.HTTPS_PASSPHRASE) {
      context.result.https.enable = true;
      context.result.https.passphrase = process.env.HTTPS_PASSPHRASE;
    }
  }
